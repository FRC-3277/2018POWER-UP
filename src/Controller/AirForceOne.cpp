...
namespace UserDefinedController
{
	...
	AirForceOne::ControllerButtonMapping AirForceOne::GetButtonMap(const std::string& givenButtonName)
	{
		...
		{
			lumberJack->eLog(std::string(__FILE__) + "; Requested button map AirForceOne not found: " + givenButtonName);
			throw ERR::CustomError(std::string(__FILE__) + "; Requested button map AirForceOne not found; " + givenButtonName);
		}
